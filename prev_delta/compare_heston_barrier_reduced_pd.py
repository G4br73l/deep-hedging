...
All parameters are IDENTICAL to compare_heston_barrier_reduced.py.
...
LOG_PATH    = os.path.join(RESULTS_DIR, "barrier_reduced_pd_log.txt")
...
def main():
    ...
    # -----------------------------------------------------------------------
    # Step 4: Instantiate models (6 features: 4 market + 2 prev_delta)
    # -----------------------------------------------------------------------
    ...
    mlp_model = MLPHedgeNetMulti(
        n_features=6, hidden_size=HIDDEN_SIZE, n_instruments=N_INSTRUMENTS
    )
    lstm_model = LSTMHedgeNetMulti(
        n_features=6, hidden_size=HIDDEN_SIZE, num_layers=2,
        n_instruments=N_INSTRUMENTS,
    )
    transformer_model = TransformerHedgeNetMulti(
        n_features=6, d_model=D_MODEL, n_heads=N_HEADS, d_ff=D_FF,
    ...
    # -----------------------------------------------------------------------
    print("\n[6] Saving results ...")
    results_path = os.path.join(RESULTS_DIR, "barrier_reduced_pd_results.pt")
    torch.save({
        ...
        "premium": premium,
        "params": {
            "experiment": "barrier_reduced_pd",
            "n_features":  6,   # 4 market features + 2 prev_delta features
    ...
    json_path = os.path.join(RESULTS_DIR, "barrier_reduced_pd_summary.json")
    ...
    plot_distribution(
        gains_dict, premium,
        os.path.join(FIGURES_DIR, "barrier_reduced_pd_gain_distributions.pdf"),
    )
    plot_decomposition(
        components_dict, premium,
        os.path.join(FIGURES_DIR, "barrier_reduced_pd_gain_decomposition.pdf"),
    )
    plot_convergence(
        history_dict = {
        ...
        save_path  = os.path.join(FIGURES_DIR, "barrier_reduced_pd_cvar_convergence.pdf"),
    )
