...
-----------------------
Variant of compare_lookback.py in which all neural-network models also receive
the previous hedge position as an additional input feature.

The full feature vector (4 features) at each step is:
    x_t = ( S_t/K,   m_t/K,   tau_t,   delta_S_{t-1} )

The extra feature delta_S_{t-1} gives each model direct access to its current
stock position.  Under proportional transaction costs this is the theoretically
correct sufficient statistic for the rebalancing decision.
...
The analytical (GSG) benchmark does NOT receive prev_delta: it outputs the
continuous-time delta formula and is used as an upper-bound reference.

All parameters, seeds, and path counts are IDENTICAL to compare_lookback.py.
...
LOG_PATH    = os.path.join(RESULTS_DIR, "lookback_pd_log.txt")
...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Sequential exact approach: f_bat are the original market features;
        # the prev_delta column is appended internally at each step inside
        # compute_gains_from_features_pd (no detach, full BPTT).
...
def main():
    ...
    # Instantiate models
    # -----------------------------------------------------------------------
    # n_features = 3 market features + 1 prev_delta feature (delta_S)
    # The analytical model still uses the 3-feature matrix (no prev_delta).
    print("\n[5] Instantiating models (n_features=4: 3 market + 1 prev_delta) ...")
    torch.manual_seed(42)
    mlp_model = MLPHedgeNet(n_features=4, hidden_size=64)
    lstm_model = LSTMHedgeNet(n_features=4, hidden_size=64, num_layers=2)
    transformer_model = TransformerHedgeNet(
        n_features=4, d_model=64, n_heads=4, d_ff=256,
    ...
    # -----------------------------------------------------------------------
    print("\n[7] Saving results ...")
    results_path = os.path.join(RESULTS_DIR, "lookback_pd_results.pt")
    torch.save({
    ...
    json_path = os.path.join(RESULTS_DIR, "lookback_pd_summary.json")
    ...
    plot_distribution(
        ...
        premium     = premium,
        save_path   = os.path.join(FIGURES_DIR, "lookback_pd_gain_distributions.pdf"),
    )
    plot_decomposition(
        components_dict = components_dict,
        premium         = premium,
        save_path       = os.path.join(FIGURES_DIR, "lookback_pd_gain_decomposition.pdf"),
    )
    plot_convergence(
        history_dict = {
        ...
        save_path  = os.path.join(FIGURES_DIR, "lookback_pd_cvar_convergence.pdf"),
    )
