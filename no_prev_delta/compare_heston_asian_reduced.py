"""
compare_heston_asian_reduced.py
--------------------------------
Ablation of compare_heston_asian.py: reduced feature engineering.

In the full experiment the feature vector at each step is

    x_t = ( S_t/S0,   VS_t/VS0,   A_t/S0,   tau_t )    (4 features)

where A_t = mean(S_1,...,S_t) is the partial arithmetic average, the
path-dependent running state that determines the Asian payoff at maturity.
In this ablation the partial average is REMOVED, leaving

    x_t = ( S_t/S0,   VS_t/VS0,   tau_t )               (3 features)

A memoryless MLP now sees only the current spot and variance-swap price,
with no information about how prices have evolved over the life of the option.
The LSTM and Transformer can still accumulate a running average through their
internal state and attention patterns respectively.

All parameters (Heston model, time grid, hyperparameters, seeds) are
IDENTICAL to compare_heston_asian.py.  The only differences are:
  - n_features = 3 instead of 4
  - A_t/S0 feature omitted
...
def gain_components_two(model, features, S, VS, payoff_fn, c):
    ...
    In particular, the variance-swap transaction cost uses |VS_t| in the
    cost term (not VS_t directly), matching the convention in gym_transformer.
    ...
    d_VS    = deltas[:, :, 1]                          # [N, T] variance-swap deltas
    dS      = S[:, 1:]  - S[:, :-1]                    # [N, T] stock increments
    dVS     = VS[:, 1:] - VS[:, :-1]                   # [N, T] VS increments
    pnl     = (d_S * dS).sum(dim=1) + (d_VS * dVS).sum(dim=1)

    # Transaction costs: delta change times instrument price.
    # For the variance swap, the contract price is |VS_t| (gym_transformer.py).
...
def main():
    ...
    # -----------------------------------------------------------------------
    print("\n[4] Instantiating models (n_features=3) ...")
    torch.manual_seed(42)
    mlp_model = MLPHedgeNetMulti(n_features=3, hidden_size=64, n_instruments=2)
    lstm_model = LSTMHedgeNetMulti(
        n_features=3, hidden_size=64, num_layers=2, n_instruments=2
    )
    ...
    # -----------------------------------------------------------------------
    print("\n[6] Saving results ...")
    results_path = os.path.join(RESULTS_DIR, "asian_reduced_results.pt")
    torch.save({
    ...
    json_path = os.path.join(RESULTS_DIR, "asian_reduced_summary.json")
    ...
    plot_distribution(
        gains_dict, premium,
        os.path.join(FIGURES_DIR, "asian_reduced_gain_distributions.pdf"),
    )
    plot_decomposition(
        components_dict, premium,
        os.path.join(FIGURES_DIR, "asian_reduced_gain_decomposition.pdf"),
    )
    plot_convergence(
        history_dict = {
