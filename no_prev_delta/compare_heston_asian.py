"""
compare_heston_asian.py
-----------------------
Experiment 2: deep hedging of an arithmetic Asian call under the Heston
stochastic volatility model, with stock and variance swap as hedging
instruments.

Three strategies are trained and evaluated:
    1. MLP         (MLPHedgeNetMulti,         memoryless feedforward)
    2. LSTM        (LSTMHedgeNetMulti,         recurrent with gated memory)
    3. Transformer (TransformerHedgeNetMulti,  causal self-attention)

Setup follows Section 5 of Buehler et al. (2019):
    dS_t = sqrt(V_t) S_t dB_t,   r = 0
    dV_t = kappa (theta - V_t) dt + epsilon sqrt(V_t) dW_t,
    corr(dB_t, dW_t) = rho dt
    Parameters: S0=100, V0=0.04, kappa=1, theta=0.04, epsilon=2, rho=-0.7

Payoff: Z = ( mean(S_1, ..., S_{N_steps}) - K )+,   K = S0 = 100

Feature vector:  x_t = ( S_t/S0,  VS_t/VS0,  A_t/S0,  tau_t )   (4 features)
    A_t = partial arithmetic average of monitoring prices seen so far.

Premium: MC discrete-path estimate (N_price=200,000, separate pricing seed).
...
from gym_transformer     import (
    build_heston_asian_feature_matrix,
    compute_gains_two_instruments,
)
...
LOG_PATH    = os.path.join(RESULTS_DIR, "asian_log.txt")
...
def gain_components_two(model, features, S, VS, payoff_fn, c):
    """
    Decompose gains into trading_pnl, costs, payoff for two-instrument hedging.
    Mirrors compute_gains_two_instruments from gym_transformer.py exactly.
...
def main():
    ...
    mlp_model = MLPHedgeNetMulti(n_features=4, hidden_size=64, n_instruments=2)
    lstm_model = LSTMHedgeNetMulti(n_features=4, hidden_size=64, num_layers=2, n_instruments=2)
    ...
    print("\n[6] Saving results ...")
    results_path = os.path.join(RESULTS_DIR, "asian_results.pt")
    torch.save({
    ...
    json_path = os.path.join(RESULTS_DIR, "asian_summary.json")
    ...
    plot_distribution(
        gains_dict, premium,
        os.path.join(FIGURES_DIR, "asian_gain_distributions.pdf"),
    )
    plot_decomposition(
        components_dict, premium,
        os.path.join(FIGURES_DIR, "asian_gain_decomposition.pdf"),
    )
    plot_convergence(
        history_dict = {
        ...
        save_path  = os.path.join(FIGURES_DIR, "asian_cvar_convergence.pdf"),
    )
