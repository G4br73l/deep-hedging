...
LOG_PATH    = os.path.join(RESULTS_DIR, "lookback_reduced_log.txt")
...
def main():
    ...
    print("\n[5] Instantiating models (n_features=2) ...")
    torch.manual_seed(42)
    mlp_model = MLPHedgeNet(n_features=2, hidden_size=64)
    lstm_model = LSTMHedgeNet(n_features=2, hidden_size=64, num_layers=2)
    transformer_model = TransformerHedgeNet(
        n_features=2, d_model=64, n_heads=4, d_ff=256,
    ...
    print("\n[7] Saving results ...")
    results_path = os.path.join(RESULTS_DIR, "lookback_reduced_results.pt")
    torch.save({
        ...
        "premium": premium,
        "params": {
            "experiment": "lookback_reduced", "n_features": 2,
    ...
    json_path = os.path.join(RESULTS_DIR, "lookback_reduced_summary.json")
    ...
    plot_distribution(
        ...
        os.path.join(FIGURES_DIR, "lookback_reduced_gain_distributions.pdf"),
    )
    plot_decomposition(
        components_dict, premium,
        os.path.join(FIGURES_DIR, "lookback_reduced_gain_decomposition.pdf"),
    )
    plot_convergence(
        history_dict = {
        ...
        save_path  = os.path.join(FIGURES_DIR, "lookback_reduced_cvar_convergence.pdf"),
    )
